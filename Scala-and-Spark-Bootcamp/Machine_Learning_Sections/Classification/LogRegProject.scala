...
val timedata = df.withColumn("Hour",hour(df("Timestamp")))

val logdata = (timedata.select(timedata("Clicked on Ad").as("label")
                ,$"Hour",$"Daily Time Spent on Site",$"Age",$"Area Income"
